...
Receiver2AudioProcessor::Receiver2AudioProcessor()
     ...
     : AudioProcessor (BusesProperties()
                     ...
                     #endif
                       ), c_write_counter(69, 0), c_read_counter(69, 0)
#endif
{
    done = false;
    loading = true;
...
void Receiver2AudioProcessor::watermarking(std::atomic<int>& read_pos, std::atomic<int>& write_pos, bool& ready)
{
    while(true)
    {
        ...
        if (ready)
        {
            ...
            {
                if (c_write_counter() - read_pos < 5)
                {
                    DBG("c_write_counter() - c_read_counter() = " << c_write_counter() - c_read_counter());
                }
                else if (c_write_counter() - c_read_counter() > 75)
                {
                    DBG("c_write_counter() - c_read_counter() = " << c_write_counter() - c_read_counter());
                }
            }
            ...
            else if (c_write_counter() < c_read_counter())
            {
                ...
                {
                    DBG("c_read_counter() - c_write_counter() = " << c_read_counter() - c_write_counter() );
                }
                else if (c_read_counter() - c_write_counter() > 75)
                {
                    DBG("c_read_counter() - c_write_counter() = " << c_read_counter() - c_write_counter());
                }
            }
...
void Receiver2AudioProcessor::client_request(corelink::client::corelink_classic_client& client, corelink::client::corelink_client_connection_info& info, corelink::core::network::channel_id_type& control_channel_id)
{
    ...
    for (int i=0; i<get_buffering_size()*2; i++)
    {
        counter_lst.push_back(0);
        tbbVector->push_back(std::vector<uint8_t>(bufferSize_ * 8));
    }
...
//                    (*tbbVector)[write_pos] = std::move(data);
                    ...
                    int w_pos = headers.get_int("counter_value") % 70;
...
//    }
    ...
    arrival_intervals.clear();
    process_intervals.clear();
