...
//        DBG("sender counter = " << counter);
        ...
        meta.append("counter_value", counter);
        
        meta2.append("bufferSize", buffer.getNumSamples());
        meta2.append("counter_value", counter);
...
//            client.send_data(hostId, std::move(data3), std::move(meta3));
        ...
        if (dir_path != "")
        {
            std::thread write_to_dat_thread(&SenderAudioProcessor::write_to_dat, this, std::move(data_to_write), counter, std::ref(dir_path));
