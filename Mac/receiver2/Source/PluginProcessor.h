...
public:
    ...
    // void write_to_dat(std::atomic<int> read_pos, std::atomic<int> write_pos, int file_counter, std::string& dir_path, std::vector<uint8_t>);
    void write_to_dat(std::atomic<int> read_pos, std::atomic<int> write_pos, std::vector<uint8_t> data);
...
private:
    ...
    std::atomic<int> audioBufferSize = 256;
    ...
    std::vector<uint8_t> *jitterBuffer = new std::vector<uint8_t>[150];
    ...
    std::vector<int> stored_read_pos;
    std::vector<int> stored_write_pos;
    
    int buffer_ctr = 0;
    std::vector<std::vector<uint8_t>>* stored_audio_data = new std::vector<std::vector<uint8_t>>(1000000);
};
